  ...
  <script>
	  $(document).ready(function(){
	     	...
	     	if ($('#user').val().trim() != '') {
	  	   	...
	  	   	ws.onopen = function (evt) {
	  			...
	  			ws.onmessage = function (evt) {		// 서버에서 넘어온값이 '2# user명 : 메세지' 라고 한다면...(앞의 ''안의 내용은 data변수에 담겨온다.')
		  			...
		  			//alert("user : " + user);
		  			let txt = evt.data.substring(evt.data.indexOf(":"));
		  			//alert("txt : " + txt);
		  ...
		  function print(user, txt) {
		  	...
		  	temp += '<div style="border:1px solid #ccc; border-radius:4px; padding:5px; text-align:left;width:100%;">'+txt+'</div>';
		  	temp += '</div>';
...
<p><br/></p>
<div class="container">
	<h1 class="page-header">대화방</h1>		
	
	<table class="table table-bordered">
		<tr>
			<td><input type="text" name="user" value="${sNickName}" id="user" class="form-control" readonly ></td>
			<td class="text-center">
				<input type="button" value="연결" id="btnConnect" class="btn btn-success"/>
				<button type="button" class="btn btn-warning" id="btnDisconnect" disabled>종료</button>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="2">
			  ...
			  <input type="color" name="chatColor" id="chatColor" />
			</td>
		</tr>
	</table>
