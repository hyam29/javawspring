...
<head>
	...
	<script>
		...
		function categoryMainCheck() {
			...
			$.ajax({
				...
				success : function(res) {
					if(res == "0") alert("동일한 상품이 등록되어 있습니다.\n확인 후 다시 입력해주세요.");
					else {
						alert("대분류 등록이 완료되었습니다.");
						location.reload();
					}
				},
				error : function() {
		...
		function categoryMainDelete(categoryMainCode) {
			let ans = confirm("선택하신 대분류 항목을 삭제하시겠습니까?");
			if(!ans) return false;
...
<div class="container">
	...
	<form name="categoryMainForm">
		...
		대분류코드(A,B,C,...)
		<input type="text" name="categoryMainCode" size="2" maxlength="2" required />, &nbsp;
		대분류명
		<input type="text" name="categoryMainName" size="8" required /> &nbsp;
		<input type="button" value="대분류등록" onclick="categoryMainCheck()" class="btn btn-sm btn-outline-primary" />
		<table class="table table-hover m-2">
			...
			<c:forEach var="mainVo" items="${mainVos}">
				<tr class="text-center">
					...
					<td><input type="button" value="삭제" onclick="categoryMainDelete('${mainVo.categoryMainCode}')" class="btn btn-sm btn-outline-danger" /></td>
				</tr>
			</c:forEach>
	...
	<form name="categoryMiddleForm">
		...
		<input type="text" name="categoryMiddleCode" size="2" maxlength="2" required />
		중분류코드(01, 02, 03, ...), &nbsp;
		중분류명
		<input type="text" name="categoryMiddleName" size="8" required /> &nbsp;
		<input type="button" value="중분류등록" onclick="categoryMiddleCheck()" class="btn btn-sm btn-outline-primary" />
		<table class="table table-hover m-2">
			<tr class="table-dark text-dark text-center">
				<th>중분류코드</th>
				<th>중분류명</th>
			...
			<c:forEach var="middleVo" items="${middleVos}">
				<tr class="text-center">
					...
					<td><input type="button" value="삭제" onclick="categoryMiddleDelete('${middleVo.categoryMiddleCode}')" class="btn btn-sm btn-outline-danger" /></td>
				</tr>
			</c:forEach>
    ...
    <table class="table table-hover m-3">
      ...
      <c:forEach var="subVo" items="${subVos}">
        <tr class="text-center">
          ...
          <td><input type="button" value="삭제" onclick="categorySubDelete('${subVo.categorySubCode}')" class="btn btn-outline-danger btn-sm"/></td>
        </tr>
      </c:forEach>
